...
public class BonusIcon : MonoBehaviour
{
    ...
    [FormerlySerializedAs("rotateSpeed")] public float 回転速度 = 720f;
    [FormerlySerializedAs("startScaleMultiplier")] public float 開始時の大きさ倍率 = 1.5f;
    [FormerlySerializedAs("endScaleMultiplier")] public float 終了時の大きさ倍率 = 0.3f;
    ...
    void Start()
    {
        ...
        if (ScoreManager.instance != null && ScoreManager.instance.スコア表示テキスト != null)
        {
            ...
            // 開始時の大きさを設定
            transform.localScale = Vector3.one * 開始時の大きさ倍率;
    ...
    void Update()
    {
        ...
        // --- 4. スケール (Lerp) ---
        float currentScale = Mathf.Lerp(開始時の大きさ倍率, 終了時の大きさ倍率, t);
